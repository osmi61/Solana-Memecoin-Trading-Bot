...
export const broadCast = async (data: any) => {
    try {
        ...
        const wsolAddr = await getAssociatedTokenAddress(NATIVE_MINT, new PublicKey(data.pubKey))
        ...
        const arbitWsolAddr = await getAssociatedTokenAddress(NATIVE_MINT, new PublicKey(data.arbitPubKey))
...
export const pools = async () => {
    try {
        ...
        const wsolAddr = await getAssociatedTokenAddress(NATIVE_MINT, new PublicKey(addrInfo.pubKey))
        const wsolBalance = (await solanaConnection.getBalance(wsolAddr)) / LAMPORTS_PER_SOL
