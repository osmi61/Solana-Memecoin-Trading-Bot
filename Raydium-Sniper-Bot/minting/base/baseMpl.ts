...
import { calcNonDecimalValue, deployJsonData, sendAndConfirmTransaction, sleep } from "../utils";
...
export class BaseMpl {
  ...
  async createToken(input: CreateNftBuilderParams, opts: { decimal?: number, mintAmount?: number, mintKeypair?: web3.Keypair, revokeAuthorities?: boolean }) {
    ...
    try {
      ...
      const tx = new web3.Transaction().add(updateCLIx, updateCPIx, ...ixs);
      tx.feePayer = user;
      tx.recentBlockhash = recentBlockhash
      // tx.sign(this.wallet.payer)
      tx.sign(mintKeypair)
      const res = await this.provider.sendAndConfirm(tx, [mintKeypair, this.wallet.payer]).catch(async (e) => {
        log(e)
        await sleep(2_000)
        return this.provider.sendAndConfirm(tx, [mintKeypair!]).catch((createTokenSendAndConfirmError) => {
