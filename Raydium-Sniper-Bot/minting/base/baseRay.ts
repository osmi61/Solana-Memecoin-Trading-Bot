...
import { ENV, RPC_ENDPOINT_DEV } from "../constants";
import { DEV_NET_RPC } from "../../config";
...
export class BaseRay {
  ...
  constructor(input: BaseRayInput) {
    this.connection = new web3.Connection(input.rpcEndpointUrl, { commitment: "confirmed" })
  ...
  async buyFromPool(input: BuyFromPoolInput): Promise<{ ixs: web3.TransactionInstruction[], signers: web3.Signer[] }> {
    ...
    if (inToken.toBase58() == NATIVE_MINT.toBase58()) {
      ...
      const tokenAccountInInfo = await this.connection.getAccountInfo(tokenAccountIn)
      if (!tokenAccountInInfo) {
        const createWSOLAtaInx = createAssociatedTokenAccountInstruction(user, tokenAccountIn, user, NATIVE_MINT)
        this.cacheIxs.push(createWSOLAtaInx)
      }
      ...
      const tokenAccountInInfo = await this.connection.getAccountInfo(tokenAccountOut)
      if (!tokenAccountInInfo) {
        const createWSOLAtaInx = createAssociatedTokenAccountInstruction(user, tokenAccountOut, user, NATIVE_MINT)
        this.cacheIxs.push(createWSOLAtaInx)
      }
...

    {
      ...
      // console.log('buy from pool', buysimRes.value.logs)
      // if (rayIxs.signers) mainTx.signatures.push(...rayIxs.signers)
    }
    return {
      ixs: [...this.cacheIxs, ...rayIxs.instructions],
  ...
  async createPool(input: CreatePoolInput, user: web3.PublicKey) {
    ...
    // ixs.push(...createPoolIxs[0].instructions)
    // signers.push(...createPoolIxs[0].signers)
