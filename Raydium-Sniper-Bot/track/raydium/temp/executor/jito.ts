...
import { BLOCKENGINE_URL, JITO_AUTH_KEYPAIR, JITO_FEE, RPC_ENDPOINT, RPC_WEBSOCKET_ENDPOINT, USE_SNIPE_LIST, solanaConnection } from "../../../../config"
...
export async function bundle(txs: VersionedTransaction[], keypair: Keypair) {
  try {
    ...
    for (let i = 0; i < txNum; i++) {
      ...
      //   tryNum++
      //   if (tryNum == 3) {
      //     logger.info("Bundling failed for 3 times in a row and stopped")
      //     break
      //   }
      //   success = await bull_dozer(newTxs, keypair)
      // }
      ...
      //   // return true
      //   successNum++
      // } else logger.warn("Bundle unsuccessful")
      // return false
